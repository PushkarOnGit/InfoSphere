...
class ArtClubScreen extends StatefulWidget {
  const ArtClubScreen({super.key});

  @override
  State<ArtClubScreen> createState() => _ArtClubScreenState();
}

class _ArtClubScreenState extends State<ArtClubScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Art Club', style: TextStyle(color: Colors.white)),
  ...
  void _voteInPoll(String noticeId, String option, String userId) async {
    await FirebaseFirestore.instance.collection('art_club_notices').doc(noticeId).update({
      ...
      builder: (context) {
        return AddNoticeDialog(
          onPost: (content, imageUrl, isPoll, pollOptions) async {
            ...
            await FirebaseFirestore.instance.collection('art_club_notices').add({
              'content': content,
              'imageUrl': imageUrl,
              'postedBy': postedBy,
  ...
  void _deleteNotice(String noticeId) async {
    await FirebaseFirestore.instance.collection('art_club_notices').doc(noticeId).delete();
  }
}